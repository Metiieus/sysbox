...
export const useProducts = () => {
  ...
  const productToDb = (product: Partial<Product>) => {
    ...
    if (
      ...
      Object.keys(product.customerPrices).length > 0
    ) {
  ...
  const createProduct = async (
    ...
    try {
      // Validar duplicação de SKU
      if (productData.sku && productData.sku.trim() !== "") {
        const existingProduct = products.find(
          (p) => p.sku.toLowerCase() === productData.sku.trim().toLowerCase(),
        );

        if (existingProduct) {
          throw new Error(
            `Já existe um produto com o SKU "${productData.sku}". Por favor, use um SKU diferente ou edite o produto existente.`,
          );
        }
      }
  ...
  const updateProduct = async (
    ...
    try {
      ...
      if (updatedProduct) {
        ...
        setProducts((prev) =>
          prev.map((p) => (p.id === productId ? merged : p)),
        );
        return merged;
      }
